...
tokens :-
  $white+  ;
  $digit+
      { \(p,_,_,s) len -> return $ TInt (read $ take len s) }
  ("0x" $hex+)
      { \(p,_,_,s) len -> return
            $ TInt (fst.head.readHex.(\(_:_:xs) -> xs)
                    $ take len s)
      }
  ("0b" $bin+)
      { \(p,_,_,s) len -> return
        $ TInt (foldl (\n c -> if c == '1' then n*2+1 else n*2) 0
                $ (\(_:_:xs) -> xs)
                $ take len s)
      }

  [$alpha] [$alpha $digit]*
      { \(p,_,_,s) len -> return $ TIdent $ take len s }
...
data Token =
	...
	| TPlus | TMinus | TMul | TDiv | TMod
        | TBinOr | TBinAnd | TBinXor | TBinNot
        | TRor | TRol
	| TLogNot | TLogAnd | TLogOr | TLogXor
        | TShl | TShr
        | TLt | TGt | TLe | TGe | TEq | TNe
	| TModifSet | TModifPlus | TModifMinus | TModifMul | TModifDiv
        | TModifMod | TModifRol | TModifRor
	| TModifShr | TModifShl
        | TModifLogAnd | TModifLogOr | TModifLogXor
        | TModifBinOr | TModifBinAnd | TModifBinXor
        | TModifInc | TModifDec
